...
var	Compiler = util.inherit(Object, {
        ...
        compile: function (pathname, data) {
            ...
            ['requires','dependencies','packages'].forEach(function(field){
               if(file.meta[field]){
                   file.meta[field] = util.unique(file.meta[field]);
               }
        ...
        mount: function (extname, pipeline) {
            ...
            pipeline = pipeline.map(function (name) {
                if(util.isFunction(name)){
                    ...
                    try {
                        return require('./pipe/' + name)(options, context);
                    } catch (e){
                        return function (options, context) {
                            throw new Error('UCC: can\'t find <' + name + '> pipeline');
                        };
                    }
                }
            });
